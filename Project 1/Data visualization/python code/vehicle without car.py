...
# Filter the data to remove "Data missing or out of range" and "Car"
df_clean = df[~df['Vehicle_Type'].isin(['Data missing or out of range', 'Car'])]

# Count the frequency of values and calculate the percentage distribution
...
norm = mcolors.Normalize(vmin=vehicle_percentage.values.min(), vmax=vehicle_percentage.values.max())
cmap = plt.cm.RdYlGn_r  

# Create a list of colors for the bars
colors = [cmap((value - vehicle_percentage.values.min()) / (vehicle_percentage.values.max() - vehicle_percentage.values.min())) for value in vehicle_percentage.values]
...
plt.savefig('vehicle_percentage.png', bbox_inches='tight')
